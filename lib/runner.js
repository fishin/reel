...
/*
internals.runCommand = function (index, command, callback) {
...
internals.saveResult = function (index, runId, result) {

    var runConfig = pail.getPail(runId);
    runConfig.commands[index] = result;
    var updateConfig = pail.updatePail(runConfig);
};
...
internals.runCommands = function (runId, commands, ready) {
    ...
    function iterate() {
        ...
        if (typeof nextCommand === 'object') {
            ...
            internals.runCommand(index, nextCommand, function (result) {

               saveResult(index, runId, result);
            }
        }
        if (code !== 0) {
