...
exports.register = function (server, options, next) {
    ...
    server.route([
        ...
        { method: 'GET', path: settings.apiPath+'/run/{runId}', config: { handler: Api.getRun, description: "get run" } },
        { method: 'GET', path: settings.apiPath+'/run/bylink/{link}', config: { handler: Api.getRunByLink, description: "get runby link" } },
        { method: 'GET', path: settings.apiPath+'/run/{runId}/start', config: { handler: Api.startRun, description: "start run" } },
        { method: 'DELETE', path: settings.apiPath+'/run/{runId}', config: { handler: Api.deleteRun, description: "delete run" } },
        { method: 'GET', path: settings.apiPath+'/run/{runId}/cancel', config: { handler: Api.cancelRun, description: "cancel run" } },
        { method: 'GET', path: settings.apiPath+'/run/{runId}/pids', config: { handler: Api.getRunPids, description: "get pids" } },
