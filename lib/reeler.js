...
module.exports = internals.Reeler = function (options) {
    ...
    internals.Reeler.settings = options;
    this.start = exports.start;
    this.recurse = exports.recurse;
    internals.Reeler.serialCall = internals.serialCall;
};
...
/*
    internals.serialCall(reelConfig.id, commands, internals.onFinish);
...
internals.onFinish = function (id, err, results) {

    //var err = this.recurse(id, 0, commands, null);
    //console.log('changed back to dir: ' + process.cwd());
    var pail = new Pail(internals.Reeler.settings.reel);
    var finishConfig = pail.getPail(id);
    var saveConfig;
    if (err) {
       finishConfig.status = 'failed';
...
internals.serialCall = function (id, commands, ready) {
    ...
    function iterate() {
        ...
        if (typeof nextCommand === 'object') {
            ...
            for (var i = 0; i < nextCommand.length; i++) {
                ...
                reelConfig.commands.push({ command: nextCommand[i] });
        ...
        process.chdir(internals.Reeler.settings.reel.pailPath + '/' + id + '/' + internals.Reeler.settings.reel.workspace);
        ...
        subprocess.on('exit', function(code, signal) {
            ...
            reelConfig.commands[index].startTime = startTime;
            reelConfig.commands[index].pid = subprocess.pid;
            reelConfig.commands[index].signal = subprocess.signal;
            reelConfig.commands[index].stdout = stdout;
            reelConfig.commands[index].stderr = stderr;
            reelConfig.commands[index].error = subprocess.error;
            reelConfig.commands[index].code = code;
            reelConfig.commands[index].finishTime = new Date().getTime();
            if (subprocess.error) {
