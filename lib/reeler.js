...
internals.recurse = function (reel_id, index, commands, err) {
    ...
    if (index < commands.length) {
        var executeResult = internals.executeSerial(commands[index]);
        //console.log('result output:\n' + JSON.stringify(executeResult, null, 4));
        var reelConfig = Store.getReelConfig(reel_id);
        if (typeof commands[index] === 'string') {
            reelConfig.commands[index].startTime = executeResult.startTime;
            ...
            reelConfig.commands[index].finishTime = executeResult.finishTime;
            err = executeResult.stderr;
            var saveConfig = Store.saveReelConfig(reelConfig);
        }
        else {
            for (var i = 0; i < executeResult.length; i++) {
                reelConfig.commands[index][i].startTime = executeResult[i].startTime;
                ...
                reelConfig.commands[index][i].finishTime = executeResult[i].finishTime;
                if (executeResult[i].stderr) {
                    err = executeResult[i].stderr;
                };
                var saveConfig = Store.saveReelConfig(reelConfig);
            }
        }
