...
exports.createReel = function (request, reply) {
   ...
   var result = Pail.savePail(internals.defaults.pailPath, config);
   reply(result);
};
...
exports.cancelReel = function (request, reply) {
   ...
   var config = Pail.getPail(internals.defaults.pailPath, request.params.id);
   // need to actually cancel the job here
   console.log('need to implement cancel');
   config.status = 'cancelled';
   Pail.savePail(internals.defaults.pailPath, config);
   reply(config);
};
...
exports.getReels = function (request, reply) {
   ...
   var reels = Pail.getPails(internals.defaults.pailPath);
   reply(reels);
};

exports.deleteReel = function (request, reply) {

   Pail.deletePail(internals.defaults.pailPath, request.params.id);
   reply('deleted');
};
